...
Office Tool Plus 由数字签名保护，运行程序时，Windows 会报告为已验证的发布者。若签名被篡改或无效，请停止使用并从[上述站点](#download)重新下载 Office Tool Plus。
