# 初次安装 {#first-deployment}

如果您选择在线安装，我们建议打开[下载后再部署](/zh-cn/usage/deploy/first-deployment.md#download-first)，以免因为网络问题而无法成功安装 Office。
...
默认情况下，将安装 64 位的 Office。更多信息请查看[体系结构](/zh-cn/usage/deploy/settings/basic.md#architecture)部分。
...
![下载后再部署的截图](/images/zh-cn/deploy/download-first.webp)
...
如果您的文件类型是镜像文件（.iso），请打开并挂载为虚拟光驱。如果您的文件类型是压缩包（例如 .zip, .7z, .rar），请将其解压到某个文件夹中。
...
![Office 安装文件的截图](/images/zh-cn/deploy/office-installation.webp)
